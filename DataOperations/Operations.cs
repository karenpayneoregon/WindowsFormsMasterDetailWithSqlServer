...
namespace DataOperations
{
    ...
    public class Operations
    {
        ...
        private void GetStateInformation()
        {
            ...
            using (var cn = new SqlConnection { ConnectionString = ConnectionString })
            {
                using (SqlCommand cmd = new SqlCommand { Connection = cn })
                {
                    ...
                    while (reader.Read())
                    {
                        StateInformation.Add(new StateItems
                        {
        ...
        /// </summary>
        public void LoadData(BindingSource pCustomerBindingSource, BindingSource pOrderBindingSource, BindingSource pOrderDetailsBindingSource)
        {
            ...
            using (var cn = new SqlConnection { ConnectionString = ConnectionString })
            {
                SqlDataAdapter da = new SqlDataAdapter(
                    "SELECT id,AccountNumber,FirstName,LastName,Address,City,State,ZipCode FROM Customer ORDER BY AccountNumber", cn);

                try
                {
                    da.Fill(ds, "Customer");
                    var dt = ds.Tables["Customer"];
                    ...
                    pCustomerBindingSource.DataSource = ds;
                    pCustomerBindingSource.DataMember = ds.Tables[0].TableName;

                    pOrderBindingSource.DataSource = pCustomerBindingSource;
                    pOrderBindingSource.DataMember = ds.Relations[0].RelationName;

                    pOrderDetailsBindingSource.DataSource = pOrderBindingSource;
                    pOrderDetailsBindingSource.DataMember = ds.Relations[1].RelationName;
        ...
        /// <summary>
        /// Remove customer (master) by primary key along with the orders (children). 
        ...
        public string GenerateInvoice(SqlConnection cn)
        {
            ...
            string result = "";
            string selectStateent = "SELECT CONVERT(VARCHAR(4), GETDATE(), 12) + " + 
                                    "RIGHT('0000' + CAST( NEXT " + 
                                    "VALUE FOR dbo.GetInvoiceNumber AS VARCHAR(3)),4)";

            using (var cmd = new SqlCommand { Connection = cn, CommandText = selectStateent })
            {
